  ...
  return (
    <div className={`flex gap-4 sm:gap-5 card rounded-card p-4 sm:p-5 ${calorieLevel}`}>
      ...
      <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1 flex-shrink-0 ml-2">
            {editing ? (
              ...
              <>
                ...
                <button onClick={() => onDelete(entry.id)} className="tap-target rounded-lg bg-transparent text-[var(--text-muted)] hover:bg-red-500/20 hover:text-red-400 transition-all opacity-100 sm:opacity-0 sm:group-hover:opacity-100 haptic">
        ...
        {entry.healthInsight && !editing && (            <p className="text-[11px] text-[var(--text-muted)] mt-1 mb-2.5 leading-relaxed">{entry.healthInsight}</p>
        )}
        {editing ? (
